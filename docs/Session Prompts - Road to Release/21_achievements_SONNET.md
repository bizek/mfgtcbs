...
- 12 achievements, all reachable in normal play, e.g.: first extraction, first boss kill, game cleared, kill milestones (500/5000), extraction count (5/25), unlock all characters (roster is 10, 12 once Druid/Cleric ship — read CharacterData.ORDER at runtime, don't hardcode the count), clear with N characters, discover N mod combos (CodexManager has discovery state), reach max depth without damage taken in a block, total gold earned, and consider combo-kit flavored ones (e.g. finish a run's boss with a combo finisher, fully allocate a passive-tree branch) if the hooks are cheap. Adjust to what stats actually exist — check ProgressionManager before finalizing.
