...
func resolve_secondary_hit(source: Node, defender: Node, raw_damage: float, knockback_force: float = 0.0) -> void:
	## Secondary damage (chain, explosion, DOT, shock) — applies armor, emits signals, but NO crit roll.
	if not is_instance_valid(defender) or not defender.has_method("take_damage"):
		return
	var armor: float = 0.0
	if defender.has_method("get_armor"):
	...
	var final_damage: float = maxf(raw_damage - armor, 1.0)
	defender.take_damage(final_damage)
	damage_dealt.emit(source, defender, final_damage, false)
	if knockback_force > 0.0 and defender.has_method("apply_knockback") and source is Node2D and defender is Node2D:
		var kb_dir: Vector2 = (defender.global_position - source.global_position).normalized()
		if kb_dir == Vector2.ZERO:
			kb_dir = Vector2(randf_range(-1.0, 1.0), randf_range(-1.0, 1.0)).normalized()
		defender.apply_knockback(kb_dir * knockback_force)
	if defender.has_method("is_dead") and defender.is_dead():
		entity_killed.emit(source, defender, defender.global_position)
