...
## SpiritGuardian — the Cleric's (The Devout) summoned companion (Spirit_Guardians package used in
## full: Summon intro, Idle, Walk, Attack, Unsummon outro). A ground-walking guardian of light with
## its OWN legs: it holds near the Devout, walks over to strike anything that strays close, and only
## trudges after the Devout once left behind — never glued to the player's movement. One at a time —
## resummon replaces (old unsummons).
##
## Mirrors the FireFamiliar/BloodElemental pet standard (CLAUDE.md): no pooling (max one alive),
## damage reads the player's live damage stat through DamageCalculator at strike time.

class_name SpiritGuardian

const ASSET_DIR: String = "res://assets/minifantasy/Minifantasy_True_Heroes_II_v1.0/Minifantasy_True_Heroes_II_Assets/Cleric/Special_Animations/Prayers/Spirit_Guardians/"
...
const WALK_SPEED: float = 46.0           ## own legs — constant-speed walking, never lerp-glued
...
const STRIKE_RANGE: float = 30.0         ## smite reach
...
func _process(delta: float) -> void:
	...
	## Resolve a pending strike (damage lands on the attack anim's smite frame).
	...
	if target:
		...
		if to_t.length_squared() <= STRIKE_RANGE_SQ:
			if _cooldown <= 0.0:
				...
				_face_toward(to_t)
				_play_dir(&"idle")   ## looming over its prey between smites
		else:
...
func _resolve_strike() -> void:
	...
	if global_position.distance_squared_to(target.global_position) > 52.0 * 52.0:
		return   ## it slipped well out of smite reach mid-swing
	var dmg: float = 25.0
...
## Early dismissal (resummon or lifetime end): Unsummon outro, then free.
func banish() -> void:
	if _state == "die":
		return
	_state = "die"
...
## Kept name-compatible with FireFamiliar/BloodElemental so player.gd can dismiss any kind generically.
func disperse() -> void:
	banish()
...
func _nearest_prey() -> Node2D:
	## Nearest live enemy within engage range of the GUARDIAN (it defends its patch of ground).
	## Gated by the 0.3s rescan — never per-frame.
...
## Shared SpriteFrames: spawn/die (1-row intro/outro), idle/move/attack (4-row directional).
static func _get_frames() -> SpriteFrames:
	if _frames_cache:
		return _frames_cache
	var frames := SpriteFrames.new()
	frames.clear_all()
	_slice_single(frames, "spawn", ASSET_DIR + "SpiritGuardianSummon.png", 13, 14.0, false)
	_slice_single(frames, "die", ASSET_DIR + "SpiritGuardianUnsummon.png", 13, 16.0, false)
	_slice_rows(frames, "idle", ASSET_DIR + "SpiritGuardianIdle.png", 16, 8.0, true)
	_slice_rows(frames, "move", ASSET_DIR + "SpiritGuardianWalk.png", 4, 10.0, true)
	_slice_rows(frames, "attack", ASSET_DIR + "SpiritGuardianAttack.png", 4, 14.0, false)
...
## The guardian's die anim has no directional rows — _play_dir falls back to the base name.
